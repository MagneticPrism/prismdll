...
bool CreatorOptionsLayer::init() {
	...
	auto title = CCLabelBMFont::create("Creator Hacks Options", "goldFont.fnt");
	title->setScale(0.9f);
	title->setPosition(winSize.width / 2, winSize.height / 2 + 110.0f);
	m_pLayer->addChild(title);

	auto creatorOptionsManager = CreatorOptionsManager::sharedManager();

	this->addToggle(
		"Test Toggle",
		creatorOptionsManager->m_objectBypass,
		nullptr
	);
	...
	auto closeBtn = gd::CCMenuItemSpriteExtra::create(
		...
		this,
		menu_selector(CreatorOptionsLayer::onClose)
	);
...
void CreatorOptionsLayer::addToggle(const char* title, bool& toggle, const char* description) {
	...
	auto toggler = gd::CCMenuItemToggler::create(
		...
		onSprite,
		this,
		menu_selector(CreatorOptionsLayer::toggleToggler)
	);
	...
	if (description) {
		...
		auto info = gd::CCMenuItemSpriteExtra::create(
			infoSprite,
			this,
			menu_selector(CreatorOptionsLayer::viewTogglerDescription)
		);
